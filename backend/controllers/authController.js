...
async function registerUser(req, res) {
    try {
        ...
        return res.status(201).json({
            _id: user._id, name: user.name, email: user.email, role: user.role, mobileNumber: user.mobileNumber,
            token: accessToken,
        });
    } catch (e) {
...
async function refreshAccessToken(req, res) {
    try {
        ...
        const user = await User.findById(payload.id).select('_id name email role mobileNumber');
        if (!user) return res.status(401).json({ message: 'User not found' });

        const newAccess = signAccessToken(user._id, user.role);
        return res.json({ token: newAccess, _id: user._id, name: user.name, email: user.email, role: user.role, mobileNumber: user.mobileNumber });
    } catch (e) {
        return res.status(500).json({ message: e.message });
    }
}
